...
describe("Limit Orders", () => {
    ...
    it("should throw an error for negative quantity in a limit order input", () => {
        expect(() => {
            limitOrderInputValid(Object.assign(Object.assign({}, testLimitOrder), { quantity: -1 }));
        }).toThrowError();
    });
    it("should throw an error for negative price in a limit order input", () => {
        expect(() => {
            limitOrderInputValid(Object.assign(Object.assign({}, testLimitOrder), { price: -100 }));
        }).toThrowError();
    });
    it("should throw an error for 0 price in a limit order input", () => {
        expect(() => {
            limitOrderInputValid(Object.assign(Object.assign({}, testLimitOrder), { price: 0 }));
        }).toThrowError();
    });
    it("should fill 1 order completely", () => {
        expect(allOrders.buy.length).toBe(4);
        expect(getAvailableQuantity(allOrders.buy)).toBe(10);
        const { resultingOrders, trades } = fillOrders(Object.assign(Object.assign({}, testLimitOrder), { side: "sell" }), allOrders);
        expect(getAvailableQuantity(resultingOrders.buy)).toBe(9);
        expect(trades.length).toBe(1);
    });
    it("should buy out the book", () => {
        expect(getAvailableQuantity(allOrders.buy)).toBe(10);
        const { resultingOrders, trades } = fillOrders(Object.assign(Object.assign({}, testLimitOrder), { side: "sell", price: 40, quantity: 11 }), allOrders);
    ...
    it("should fill 1 order partially", () => {
        expect(allOrders.buy.length).toBe(4);
        const { resultingOrders, trades } = fillOrders(Object.assign(Object.assign({}, testLimitOrder), { side: "sell", quantity: 0.5 }), allOrders);
    ...
    it("should add a new order for a different price", () => {
        ...
        const newOrders = addToOrders(Object.assign(Object.assign({}, testLimitOrder), { price: 100.7 }), allOrders);
    ...
    it("should throw an error for 0 quantity in a limit order input", () => {
        expect(() => {
            limitOrderInputValid(Object.assign(Object.assign({}, testLimitOrder), { quantity: 0 }));
        }).toThrowError();
    });
    ...
    it("should be able to completely fill the order", () => {
        const canFill = canFillOrder(Object.assign(Object.assign({}, testLimitOrder), { side: "sell" }), allOrders);
        expect(canFill).toBe(true);
    });
    it("should be able to partially fill the order", () => {
        const canFill = canFillOrder(Object.assign(Object.assign({}, testLimitOrder), { side: "sell", quantity: 7 }), allOrders);
        expect(canFill).toBe(true);
    });
});
